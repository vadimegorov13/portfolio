<script lang="ts">
  ...
  const contacts: Link[] = [
    ...
    {
      href: ' https://profile.indeed.com/?hl=en_US&co=US&from=gnav-menu-homepage&_ga=2.198835887.1699684602.1673660855-1317022637.1668652652&_gac=1.262700414.1673129916.Cj0KCQiAzeSdBhC4ARIsACj36uGhZXk1_vv4B5qwVhyCoX2xgUYjjsbYdksOna-TSpas3oBgGRVR7DwaAldlEALw_wcB',
      icon: 'cib:indeed',
      name: 'Indeed',
      description:
...
<div>
  ...
  <p class="text-zinc-400">Let's be friends,</p>

  <div class="flex flex-col gap-10 mt-10 max-w-[40rem] text-zinc-400">
    <p>
  ...
  <div class="flex flex-col mt-10  text-sm group">
    {#each contacts as contact}
      <div class="flex flex-row mt-8 text-zinc-400  ">
        <div class="w-full flex flex-row">
          <Icon icon={contact.icon} class="text-2xl smd:text-6xl mr-2 " />
          ...
          <div class="flex flex-col justify-center">
            ...
            {:else}
              <p
                class="group-hover:gr-animation justify-end border border-border border-x-0 border-t-0"
              >
                {contact.name}
              </p>
            {/if}
            <p class="w-full mt-2">{contact.description}</p>
          </div>
        </div>
      </div>
    {/each}
  ...
  <div class="flex flex-col">
    <h1 class="text-xl font-semibold mt-10">
      <span class="text-primary">#</span>email-me
    </h1>
    <form
      action="https://api.staticforms.xyz/submit"
      method="post"
      class="flex flex-col gap-3 content m-4"
    >
      <input
        type="hidden"
        name="accessKey"
        value="440fea79-fdf4-44b4-b74b-8d4a4c820ee3"
      />
      <div>
      ...
      <div class="flex flex-col justify-center items-center">
        <input
          type="submit"
          value="Send"
          class="w-14 text-lg hover:text-primary cursor-pointer"
        />
      </div>
    </form>
  </div>
